...
public final class RefreshUtils {
    ...
    protected int getStateForPackage(String packageName) {
        ...
        if (modes.length >= 4) {
            if (modes[0].contains(packageName + ",")) return STATE_STANDARD;
            if (modes[1].contains(packageName + ",")) return STATE_MEDIUM;
            if (modes[2].contains(packageName + ",")) return STATE_HIGH;
            if (modes[3].contains(packageName + ",")) return STATE_EXTREME;
        }
    ...
    getOldRate(); // make sure defaults are loaded
